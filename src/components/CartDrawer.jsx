...
import { ShoppingCart, X, Trash2, Lock } from 'lucide-react'; // Quitamos Minus y Plus de los imports
            ...
            <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-fade-in-right">
                ...
                <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-noviblue text-white">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="font-bold text-lg">Tu Carrito ({cartItems.length})</h2>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4">
                    {cartItems.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-center text-gray-400">
                            ...
                            <button onClick={onClose} className="mt-4 text-noviblue font-bold hover:underline">Continuar comprando</button>
                        </div>
                    ) : (
                        cartItems.map((item) => (
                            <div key={item.id} className="flex gap-4 border-b border-gray-50 pb-4 last:border-0">
                                ...
                                <div className="flex-1 flex flex-col justify-between">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-400 mt-1 uppercase">{item.brand}</p>
                                    </div>
                                    <div className="flex justify-between items-end mt-2">
                                        {/* CAMBIO: Solo mostramos Cant: 1 estático */}
                                        ...
                                        <div className="text-right">
                                            <p className="font-extrabold text-noviblue">{format(item.price)}</p>
                                        </div>
                                    </div>
                                </div>
                                
                                {/* Eliminar */}
                                <button onClick={() => onRemove(item.id)} className="text-gray-300 hover:text-novired self-start p-1 transition">
                                    <Trash2 size={16} />
                                </button>
                            </div>
                        ))
                ...
                {cartItems.length > 0 && (
                    <div className="p-6 border-t border-gray-100 bg-gray-50">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <span className="text-2xl font-extrabold text-gray-900">{format(total)}</span>
                        </div>
                        <p className="text-xs text-gray-400 text-center mb-4">Envío calculado al finalizar</p>
                        <button onClick={onCheckout} className="w-full bg-noviyellow text-gray-900 font-extrabold py-3.5 rounded-lg shadow-lg hover:bg-yellow-400 transition flex items-center justify-center gap-2 transform hover:-translate-y-1">
